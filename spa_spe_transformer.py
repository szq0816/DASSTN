...
class SpatialGate(nn.Module):
    ...
    def forward(self, x, H=11, W=11):
        ...
        x2 = x2.permute(0, 2, 1).view(B, C, N // 11, N // 11)
        x = x1 * x2
        return x  # x(64, 100, 11, 11)

# model = SpatialGate(dim=100)
# model.eval()
# print(model)
...
class SGFN(nn.Module):
    ...
    def forward(self, x, H=11, W=11):
        ...
        x = x.permute(0, 2, 1).view(64, -1, H, W)
        x = self.sg(x, H, W)
        x = self.dropout(x)
...
class Spatial_Attention(nn.Module):
    ...
    def __init__(self, dim, idx, split_size=[3, 3], dim_out=None, num_heads=1, attn_drop=0., proj_drop=0.,
    ...
    def forward(self, qkv, H=11, W=11, mask=None):
        """
...
class Adaptive_Spatial_Attention(nn.Module):
    ...
    def __init__(self, dim, num_heads,
                 reso=11, split_size=[3, 3], shift_size=[1, 2], qkv_bias=False, qk_scale=None,
    ...
    def calculate_mask(self, H=11, W=11):
    ...
    def forward(self, x, H=11, W=11):
        ...
        # for square windows (e.g., 8x8), window-0 and window-1 can be merged
...
class SpatialBlock(nn.Module):
    def __init__(self, dim, num_heads, reso=11, split_size=[3, 3], shift_size=[1, 2], expansion_factor=4.,
        ...
        self.spa_attn = Adaptive_Spatial_Attention(dim, num_heads=num_heads, reso=reso, split_size=split_size,
    ...
    def forward(self, x, x_size=(11, 11)):
        """
        ...
        x = x.flatten(2).permute(0, 2, 1)
        x1 = self.norm1(x)
        x2 = x + self.drop_path(self.spa_attn(x1, H, W))
        x3 = self.norm2(x2)
        x3 = x3.permute(0, 2, 1).view(64, -1, H, W)
        x4 = self.ffn(x3, H, W)
        x5 = x2 + self.drop_path(x4)

        return x5


class SpectralBlock(nn.Module):
    def __init__(self, dim, num_heads, expansion_factor=4., qkv_bias=False, qk_scale=None, drop=0.,
                 attn_drop=0., drop_path=0., act_layer=nn.GELU, norm_layer=nn.LayerNorm):
        super().__init__()

        self.norm1 = norm_layer(dim)

        self.spe_attn = Adaptive_Channel_Attention(dim, num_heads=num_heads,
    ...
    def forward(self, x, x_size=(11, 11)):
        ...
        x = x.flatten(2).permute(0, 2, 1)
        y1 = self.norm1(x)
        y2 = x + self.drop_path(self.spe_attn(y1, H, W))
        y3 = self.norm2(y2)
        y3 = y3.permute(0, 2, 1).view(64, -1, H, W)
        y4 = self.ffn(y3, H, W)
        y5 = y2 + self.drop_path(y4)
